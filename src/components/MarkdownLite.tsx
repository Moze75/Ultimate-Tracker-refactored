...
export function parseMarkdownLite(md: string, ctx: MarkdownCtx): React.ReactNode[] {
  ...
  // On remplace globalement les entités HTML &lt; et &gt; par < et > 
  // Cela permet au parser de reconnaître les balises <!-- BOX --> qui arrivent encodées depuis la DB.
  const normalized = (md || '')
    .replace(/&lt;/g, '<')
    .replace(/&gt;/g, '>');
  ...
  while (i < lines.length) {
    ...
    while (
      ...
      !/^\s*---+\s*$/.test(lines[i]) &&
      !/^\s*\*\*[^*]+\*\*/.test(lines[i]) &&
      !lines[i].match(/^\s*<!--/) // Stop paragraphe si on rencontre un commentaire/box
    ) {
      buff.push(lines[i]);
      i++;
    }
