...
export function DiceSettingsModal({ open, onClose, settings, onSave }: DiceSettingsModalProps) {
  ...
  return (
    ...
    <div className="fixed inset-0 z-50 bg-black/50">
      {/* ✅ Wrapper pour le scroll si nécessaire */}
      <div className="fixed inset-0 overflow-y-auto">
        {/* ✅ Conteneur flex pour centrage */}
        <div className="min-h-full flex items-center justify-center p-4">
          {/* ✅ Modal card avec hauteur max */}
          <div className="bg-gray-800 rounded-lg border border-gray-700 shadow-xl w-full max-w-md max-h-[90vh] flex flex-col">
            ...
            {/* Footer - fixe, seulement pour l'onglet paramètres */}
            {activeTab === 'settings' && (
    ...
    <div className="space-y-4">
      ...
      {!localSettings.theme && (
        <div>
          ...
          <div className="grid grid-cols-6 gap-2 mb-3">
            <button onClick={() => handleChange('themeColor', '#ff0000')} className="w-10 h-10 rounded-lg bg-red-500 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#ff8800')} className="w-10 h-10 rounded-lg bg-orange-500 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#ffd700')} className="w-10 h-10 rounded-lg hover:ring-2 ring-white transition-all" style={{backgroundColor: '#ffd700'}} type="button" />
            <button onClick={() => handleChange('themeColor', '#00ff00')} className="w-10 h-10 rounded-lg bg-green-500 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#0088ff')} className="w-10 h-10 rounded-lg bg-blue-500 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#8800ff')} className="w-10 h-10 rounded-lg bg-purple-600 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#ff00ff')} className="w-10 h-10 rounded-lg bg-pink-500 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#00ffff')} className="w-10 h-10 rounded-lg bg-cyan-400 hover:ring-2 ring-white transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#8B4513')} className="w-10 h-10 rounded-lg hover:ring-2 ring-white transition-all" style={{backgroundColor: '#8B4513'}} type="button" />
            <button onClick={() => handleChange('themeColor', '#C0C0C0')} className="w-10 h-10 rounded-lg hover:ring-2 ring-white transition-all" style={{backgroundColor: '#C0C0C0'}} type="button" />
            <button onClick={() => handleChange('themeColor', '#ffffff')} className="w-10 h-10 rounded-lg bg-white border border-gray-600 hover:ring-2 ring-purple-500 transition-all" type="button" />
            <button onClick={() => handleChange('themeColor', '#000000')} className="w-10 h-10 rounded-lg bg-black border border-gray-600 hover:ring-2 ring-white transition-all" type="button" />
          </div>
          
          {/* Sélecteur de couleur personnalisé */}
          <div className="flex gap-2 items-center">
            <input type="color" value={localSettings.themeColor} onChange={(e) => handleChange('themeColor', e.target.value)} className="w-16 h-10 rounded cursor-pointer border border-gray-600" />
            <input type="text" value={localSettings.themeColor} onChange={(e) => handleChange('themeColor', e.target.value)} className="flex-1 px-3 py-2 bg-gray-700 border border-gray-600 rounded-lg text-white text-sm focus:ring-2 focus:ring-purple-500" placeholder="#ff0000" />
          </div>
        </div>
      )}
      ...
      <div className="flex items-center justify-between">
        ...
        <button
          ...
          className={`relative w-12 h-6 rounded-full transition-colors ${localSettings.soundsEnabled ? 'bg-purple-600' : 'bg-gray-600'}`}
        >
          <span className={`absolute top-0.5 left-0.5 w-5 h-5 bg-white rounded-full transition-transform ${localSettings.soundsEnabled ? 'translate-x-6' : 'translate-x-0'}`} />
        </button>
      </div>
      ...
      <div>
        ...
        <input type="range" min="3" max="10" step="0.5" value={localSettings.baseScale} onChange={(e) => handleChange('baseScale', parseFloat(e.target.value))} className="w-full accent-purple-600" />
      ...
      <div>
        ...
        <input type="range" min="0.5" max="2" step="0.1" value={localSettings.gravity} onChange={(e) => handleChange('gravity', parseFloat(e.target.value))} className="w-full accent-purple-600" />
      </div>
      ...
      <div>
        ...
        <input type="range" min="0.5" max="3" step="0.1" value={localSettings.strength} onChange={(e) => handleChange('strength', parseFloat(e.target.value))} className="w-full accent-purple-600" />
      </div>
      ...
      <div>
        ...
        <input type="range" min="0" max="100" step="5" value={localSettings.volume} onChange={(e) => handleChange('volume', parseInt(e.target.value))} className="w-full accent-purple-600" disabled={!localSettings.soundsEnabled} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="space-y-3">
      ...
      <div className="flex items-center justify-between pb-2 border-b border-gray-700">
        ...
        <button onClick={onClearHistory} className="px-3 py-1 text-xs bg-red-600/20 hover:bg-red-600/30 text-red-400 rounded-lg transition-colors flex items-center gap-1">
      ...
      <div className="space-y-2">
        {history.map((entry) => (
          <div key={entry.id} className="bg-gray-700/50 rounded-lg p-3 border border-gray-600/50 hover:border-purple-500/50 transition-colors">
            <div className="flex items-start justify-between mb-2">
              ...
              <button onClick={() => onRemoveEntry(entry.id)} className="p-1 hover:bg-red-600/20 rounded transition-colors text-gray-500 hover:text-red-400" title="Supprimer">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1 text-xs text-gray-400">
                <p>{entry.diceFormula} → [{entry.rolls.join(', ')}] = {entry.diceTotal}</p>
                {entry.modifier !== 0 && (
                  <p className="text-orange-400">{entry.modifier >= 0 ? '+' : ''}{entry.modifier}</p>
                )}
              </div>
            </div>
          </div>
