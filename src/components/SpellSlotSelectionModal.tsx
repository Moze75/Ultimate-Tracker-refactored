...
export function SpellSlotSelectionModal({
  ...
  const isWarlock = player.class === 'Occultiste' || player.secondary_class === 'Occultiste';
  const combinedSlots = combineSpellSlots(player);

  const getAvailableSlots = (level: number): { max: number; used: number; available: number } => {
    if (!combinedSlots) return { max: 0, used: 0, available: 0 };

    if (isWarlock && level === (combinedSlots.pact_level || 1)) {
      const max = combinedSlots.pact_slots || 0;
      const used = combinedSlots.used_pact_slots || 0;
    ...
    const max = combinedSlots[levelKey] || 0;
    const used = combinedSlots[usedKey] || 0;
    const available = Math.max(0, max - used);
  ...
  const getAvailableLevels = (): number[] => {
    if (isWarlock) {
      const pactLevel = combinedSlots?.pact_level || 1;
      const pactSlots = combinedSlots?.pact_slots || 0;
      const usedPactSlots = combinedSlots?.used_pact_slots || 0;
        ...
        <div className="p-4 flex-1 overflow-y-auto">
          ...
          <div className="space-y-2 mb-4">
            {availableLevels.map(level => {
              ...
              const isPactSlot = isWarlock && level === (combinedSlots?.pact_level || 1);
